...
console.log(chalk.bold.bgBlack.bgRed.underline("\n\tWellcome To Adventure Game "), chalk.yellowBright("By"), chalk.bold.bgBlack.bgRed.underline("SYED SAUD ALI\n"));
async function startGame() {
    ...
    console.log(chalk.bold(`\n\tPerson Name: ${chalk.cyan.underline.bold.italic(p1.name)} | Fuel:${chalk.bold.green(p1.fuel)} ${chalk.bold.red("\n\t\t\tVS\n")} \tOpponent: ${chalk.cyan.underline.italic(o1.name)} | Fuel:${chalk.bold.green(o1.fuel)}\n`));
    do {
        ...
        if (fightOption.chooseOption === "Attack") {
            ...
            if (count > 0) {
                p1.fuelDecrease();
                console.log(chalk.bold(`\n\tPerson Name: ${chalk.bold.red(p1.name)} | Fuel: ${chalk.bold.underline.red(p1.fuel)} ${chalk.bold.red(" Damaged")}`));
                console.log(chalk.bold(`\tOpponent: ${chalk.bold.green(o1.name)} | Fuel: ${chalk.bold.underline.green(o1.fuel)} ${chalk.bold.green(" Attacked")}\n`));
            }
            if (p1.fuel <= 0) {
            ...
            if (count <= 0) {
                o1.fuelDecrease();
                console.log(chalk.bold(`\n\tOpponent: ${chalk.bold.red(o1.name)} | Fuel: ${chalk.bold.underline.red(o1.fuel)} ${chalk.bold.red(" Damaged")}`));
                console.log(chalk.bold(`\tPerson Name: ${chalk.bold.green(p1.name)} | Fuel: ${chalk.bold.underline.green(p1.fuel)} ${chalk.bold.green(" Attacked")}\n`));
            }
            if (o1.fuel <= 0) {
